...
import { MINIMAX_BASE_URL, MINIMAX_DEFAULT_MODEL } from '@/lib/providers/minimax'
...
const getSavedModel = (
  ...
  if (provider === 'anthropic') {
    ...
    return localStorage.getItem(MINIMAX_MODEL_KEY) || MINIMAX_DEFAULT_MODEL
  }
  return undefined
}
    ...
    try {
      ...
      const request: LaunchSessionRequest = {
        ...
        ...(config.provider === 'minimax' && config.minimaxApiKey
          ? {
              proxy_enabled: true,
              proxy_base_url: MINIMAX_BASE_URL,
