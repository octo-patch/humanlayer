...
import type { LaunchSessionRequest, ModelProvider } from '@/lib/daemon/types'
...
import { DEFAULT_MINIMAX_BASE_URL, DEFAULT_MINIMAX_MODEL, normalizeMiniMaxBaseUrl } from '@/lib/minimax'
...
const MINIMAX_BASE_URL_KEY = 'humanlayer-minimax-base-url'
...
const getSavedModel = (provider: ModelProvider): string | undefined => {
  if (provider === 'anthropic') {
    ...
    return localStorage.getItem(MINIMAX_MODEL_KEY) || DEFAULT_MINIMAX_MODEL
  }
  return undefined
}
...
const getSavedMiniMaxBaseUrl = (): string => {
  return normalizeMiniMaxBaseUrl(localStorage.getItem(MINIMAX_BASE_URL_KEY))
}
...
export const useSessionLauncher = create<LauncherState>((set, get) => ({
  ...
  setConfig: config => {
    ...
    if (config.miniMaxBaseUrl) {
      localStorage.setItem(MINIMAX_BASE_URL_KEY, normalizeMiniMaxBaseUrl(config.miniMaxBaseUrl))
    }
    ...
    try {
      ...
      const request: LaunchSessionRequest = {
        ...
        ...(config.provider === 'minimax'
          ? {
              proxy_enabled: true,
              proxy_base_url: normalizeMiniMaxBaseUrl(
                config.miniMaxBaseUrl || DEFAULT_MINIMAX_BASE_URL,
              ),
              proxy_model_override: config.model || DEFAULT_MINIMAX_MODEL,
