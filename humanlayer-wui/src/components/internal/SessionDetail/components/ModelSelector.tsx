...
function ModelSelectorContent({
  ...
  const handleProviderChange = (newProvider: 'anthropic' | 'openrouter' | 'baseten' | 'minimax') => {
    ...
    if (newProvider === 'anthropic') {
      ...
      // MiniMax exposes a fixed set of model IDs, so preselect the default one
  ...
  // Providers that are reached through the proxy and therefore need an API key
  const isProxyProvider = provider === 'openrouter' || provider === 'baseten' || provider === 'minimax'
  const proxyProviderLabel =
    provider === 'baseten' ? 'Baseten' : provider === 'minimax' ? 'MiniMax' : 'OpenRouter'
  const canApplyProxyProvider =
    provider === 'baseten'
      ? canApplyBaseten
      : provider === 'minimax'
        ? canApplyMiniMax
  ...
  const apiKeyPlaceholder =
    provider === 'openrouter' ? 'sk-or-...' : `Enter ${proxyProviderLabel} API key...`
  ...
  const handleApply = async () => {
    if (
      ...
      const providerLabel =
        provider === 'baseten' ? 'Baseten' : provider === 'minimax' ? 'MiniMax' : 'OpenRouter'
      toast.error(`Model name is required for ${providerLabel}`)
      return
    }
  ...
  return (
    <div ref={focusTrapRef}>
      ...
      <div className="mt-6 space-y-4">
          ...
          {provider === 'openrouter' ? (
            ...
            >
              <SelectTrigger id="model" className="w-full">
                <SelectValue placeholder={MINIMAX_DEFAULT_MODEL} />
              </SelectTrigger>
              <SelectContent>
        ...
        {/* API Key Status for the proxy-backed providers */}
        {isProxyProvider && (
          <div className="space-y-2">
            {!showApiKeyInput ? (
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  ) : canApplyProxyProvider ? (
                    <CheckCircle className="h-4 w-4 text-green-500" />
                  ) : (
                    <AlertCircle className="h-4 w-4 text-destructive" />
                  )}
                  <span
                    className={canApplyProxyProvider ? 'text-muted-foreground' : 'text-destructive'}
                  >
                    {isCheckingConfig
                      ? `Checking ${proxyProviderLabel} configuration...`
                      : canApplyProxyProvider
                        ? `${proxyProviderLabel} API key is configured`
                        : `${proxyProviderLabel} API key required`}
                  </span>
                </div>
                <Button
                  variant="ghost"
              ...
              <div className="space-y-2">
                <Label htmlFor="api-key">{proxyProviderLabel} API Key</Label>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button
            ...
            disabled={
              ...
              (provider === 'baseten' && !canApplyBaseten) ||
              (provider === 'minimax' && !canApplyMiniMax) ||
              (isProxyProvider && !customModel.trim())
            }
            size="sm"
          >
